<template>
  <AppLayout title="Cycle Count">
    <div class="min-h-screen bg-gray-50 dark:bg-gray-950 p-2 sm:p-4 md:p-6 transition-colors duration-300">
        <div class="bg-white dark:bg-gray-900 p-4 rounded-lg shadow mb-6 border border-gray-100 dark:border-gray-800">
            <div class="flex flex-col md:flex-row justify-between gap-4">
                <div class="flex flex-wrap gap-2 w-full md:w-3/4">

                    <select v-model="filterForm.status" @change="applyFilter" class="w-full sm:w-40 text-sm border rounded bg-gray-50 dark:bg-gray-800 py-1.5 dark:text-gray-200 dark:border-gray-700">
                    ...
                    <select v-model="filterForm.category" @change="applyFilter" class="w-full sm:w-48 text-sm border rounded bg-gray-50 dark:bg-gray-800 py-1.5 dark:text-gray-200 dark:border-gray-700">
                    ...
                    <div class="relative flex-grow sm:flex-grow-0 flex gap-2">
                        ...
                        <div class="relative w-full sm:w-64">
                            <input 
                                ...
                                class="w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all dark:bg-gray-800 dark:text-gray-200 dark:border-gray-700"
                            >
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div class="bg-white dark:bg-gray-900 rounded-xl shadow p-4 border-l-4 border-blue-500">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase font-bold">Total SKU (Stock)</p>
                        <h3 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mt-1">{{ formatNumber(statistics.total_sku) }}</h3>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl shadow p-4 border-l-4 border-green-500">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase font-bold">Sudah Opname (Bulan Ini)</p>
                        <h3 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mt-1">
                            {{ formatNumber(statistics.counted_items) }} 
                            <span class="text-xs font-normal text-gray-400 dark:text-gray-500">/ {{ formatNumber(statistics.total_sku) }}</span>
                        </h3>
                        <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 mt-2">
                            <div class="bg-green-500 h-1.5 rounded-full" :style="`width: ${statistics.progress_percentage}%`"></div>
                        </div>
                    </div>
                    <div class="p-2 bg-green-100 dark:bg-green-900/40 rounded-lg text-green-600 dark:text-green-400">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    </div>
                </div>
            </div>

            <div class="bg-white dark:bg-gray-900 rounded-xl shadow p-4 border-l-4" :class="statistics.avg_accuracy >= 98 ? 'border-indigo-500' : 'border-red-500'">
                <div class="flex justify-between items-start">
                    <div>
                      <p class="text-xs text-gray-500 dark:text-gray-400 uppercase font-bold">Rata-rata Akurasi</p>
                      <h3 class="text-2xl font-bold mt-1" :class="statistics.avg_accuracy >= 98 ? 'text-indigo-600 dark:text-indigo-400' : 'text-red-600 dark:text-red-400'">
                          {{ statistics.avg_accuracy }}%
                      </h3>
                      <p class="text-[10px] text-gray-400 dark:text-gray-500 mt-1">Target: >98%</p>
                  </div>
                  <div class="p-2 rounded-lg" :class="statistics.avg_accuracy >= 98 ? 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400' : 'bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400'">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl shadow p-4 border-l-4 border-orange-500 cursor-pointer hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-colors" @click="setFilterUncounted">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase font-bold">Belum Pernah Opname</p>
                        <h3 class="text-2xl font-bold text-orange-600 dark:text-orange-400 mt-1">{{ formatNumber(statistics.never_counted) }}</h3>
        ...
        <div class="overflow-x-auto bg-white dark:bg-gray-900 shadow-md rounded-lg">
            <table class="w-full border-collapse border border-blue-300 dark:border-blue-700 text-sm">
            <thead class="bg-blue-200 dark:bg-blue-900 text-gray-800 dark:text-gray-200 font-semibold text-center">
                <tr>
                ...
                <tr>
                <th class="border border-blue-400 dark:border-blue-700 p-2 bg-blue-100 dark:bg-blue-800/50 w-32">Scan Serial</th>
                <th class="border border-blue-400 dark:border-blue-700 p-2 bg-blue-100 dark:bg-blue-800/50 w-24">Scan Bin</th>
                <th class="border border-blue-400 dark:border-blue-700 p-2 bg-blue-100 dark:bg-blue-800/50 w-20">Qty</th>
                <th class="border border-blue-400 dark:border-blue-700 p-2 bg-blue-100 dark:bg-blue-800/50 w-20">Acc</th>
                <th class="border border-blue-400 dark:border-blue-700 p-2 bg-blue-100 dark:bg-blue-800/50 w-20">In Acc</th>
                <th class="border border-blue-400 dark:border-blue-700 p-2 bg-yellow-100 dark:bg-yellow-900/50 w-40">Note SPV</th>
                <th class="border border-blue-400 dark:border-blue-700 p-2 bg-yellow-100 dark:bg-yellow-900/50 w-20">Action</th>
                </tr>
            </thead>
            <tbody>
                <template v-for="(item, index) in form.items" :key="index">
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 text-center text-gray-700 dark:text-gray-300" :class="isSelected(item) ? 'bg-orange-50 dark:bg-orange-900/20' : ''">
                ...
                <td class="border border-gray-300 dark:border-gray-700 p-2 bg-gray-50 dark:bg-gray-800 text-xs font-mono font-bold text-blue-900 dark:text-blue-100">
                    {{ extractSerial(item.serial_number) }}
                </td>
                <td class="border border-gray-300 dark:border-gray-700 p-2 text-left bg-gray-50 dark:bg-gray-800 text-xs truncate max-w-[150px]" :title="item.product_name">
                    {{ item.product_name }}
                ...
                <td class="border border-gray-300 dark:border-gray-700 p-2 bg-gray-50 dark:bg-gray-800 text-right whitespace-nowrap">
                ...
                <td class="border border-gray-300 dark:border-gray-700 p-2 bg-gray-50 dark:bg-gray-800 font-bold">{{ item.location }}</td>
                ...
                <td class="border border-gray-300 p-1">
                    ...
                    :class="isUnlocked(item) ? 'bg-white dark:bg-gray-900 border-blue-500 text-blue-800 dark:text-blue-100' : 'bg-gray-100 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-400 cursor-not-allowed'" placeholder="0">
                </td>
                ...
                <td class="border border-gray-300 dark:border-gray-700 p-1 bg-yellow-50 dark:bg-yellow-900/30">
                    <input type="text" v-model="item.spv_note" 
                        class="w-full text-xs border border-yellow-300 rounded px-1 py-1 bg-white dark:bg-gray-900 dark:border-yellow-700 dark:text-gray-200 focus:ring-yellow-500 focus:border-yellow-500" 
                        placeholder="Catatan SPV...">
                </td>
                <td class="border border-gray-300 dark:border-gray-700 p-1 bg-yellow-50 dark:bg-yellow-900/30 text-center align-middle">
                ...
                <tr v-if="item.status === 'APPROVED' || (item.history && item.history_count > 0)" class="bg-gray-50 dark:bg-gray-800">
                <td colspan="11" class="border border-gray-300 dark:border-gray-700 p-2"> <div class="flex items-center justify-between">
                    <button 
                    ...
                    <table class="w-full text-xs border border-gray-300 dark:border-gray-700">
                        ...
                        <tbody>
                        <tr v-for="(h, hIndex) in item.history" :key="hIndex" class="hover:bg-gray-100 dark:hover:bg-gray-600">
                            <td class="border border-gray-300 dark:border-gray-700 p-2 text-center text-gray-800 dark:text-gray-200">{{ h.count_date }}</td>
        ...
        <div v-if="showBulkModal" class="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div class="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full max-w-2xl mx-4 flex flex-col max-h-[90vh] border border-gray-200 dark:border-gray-800">
                <div class="p-4 border-b dark:border-gray-800 bg-gray-50 dark:bg-gray-800 flex justify-between items-center rounded-t-lg">
                    <h3 class="text-lg font-bold text-gray-800 dark:text-gray-100">Konfirmasi Cycle Count Ulang</h3>
                    ...
                    <div class="border rounded-lg overflow-hidden dark:border-gray-700">
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700 text-gray-700 dark:text-gray-300">
                                <tr v-for="(item, idx) in selectedItems" :key="idx" class="hover:bg-gray-50 dark:hover:bg-gray-800">
            ...
            <div class="p-4 border-t dark:border-gray-800 bg-gray-50 dark:bg-gray-800 flex justify-end gap-3 rounded-b-lg">
        ...
        <div v-if="showScanner" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm transition-opacity">
            <div class="bg-white dark:bg-gray-900 rounded-lg shadow-2xl w-full max-w-sm mx-4 overflow-hidden relative animate-fade-in-down border border-gray-200 dark:border-gray-800">
                <div class="bg-gray-100 dark:bg-gray-800 px-4 py-3 border-b dark:border-gray-700 flex justify-between items-center">
                    <h3 class="font-bold text-gray-800 dark:text-gray-100 text-sm">Scan: <span class="text-blue-600 dark:text-blue-400">{{ scanningField === 'scan_serial' ? 'Serial Number' : (scanningField === 'scan_bin' ? 'Lokasi Bin' : 'Cari Material') }}</span></h3>
                    <button @click="closeScanner" class="text-gray-400 hover:text-red-500 font-bold text-2xl leading-none">&times;</button>
                </div>
                <div class="p-0 bg-white dark:bg-gray-900 relative">
                ...
                <div class="bg-white dark:bg-gray-900 px-4 py-3 border-t dark:border-gray-700 text-center">
                    <button @click="closeScanner" class="w-full bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 font-semibold py-2 px-4 rounded shadow">Batal Scan</button>
                </div>
            </div>
        </div>

        <div v-if="showErrorModal" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div class="bg-white dark:bg-gray-900 rounded-lg shadow-xl max-w-sm w-full mx-4 p-6 text-center animate-bounce-short border border-gray-200 dark:border-gray-800">
                <div class="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-red-100 dark:bg-red-900/30 mb-4">
                <svg class="h-6 w-6 text-red-600 dark:text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                </svg>
                </div>
                <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-100 mb-2">Scan Ditolak!</h3>
